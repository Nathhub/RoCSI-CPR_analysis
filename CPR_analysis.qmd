...
```
::: {.callout-note}
### 💬 Discussion — Dominant arthropods genera
...
df <- df %>%
    mutate(
        genus_plot = ifelse(OTU %in% top10_taxa, as.character(Genus), "Other")
    ) %>%
    group_by(Longitude, genus_plot) %>%
    summarise(Abundance = sum(Abundance), .groups = "drop")
...
top10_genus_ordered <- tax_tab %>%
    filter(OTU %in% top10_taxa) %>%
    pull(Genus)

top10_genus_ordered <- unique(top10_genus_ordered[!is.na(top10_genus_ordered) & top10_genus_ordered != ""])

cols_top10 <- setNames(my_colors[1:length(top10_genus_ordered)], top10_genus_ordered)
fill_cols  <- c(cols_top10, Other = "grey80")

df$genus_plot <- factor(df$genus_plot, levels = c(top10_genus_ordered, "Other"))

# 7) Plot
ggplot(df, aes(x = Longitude, y = Abundance, fill = genus_plot)) +
    geom_bar(stat = "identity", position = "stack") +
    scale_fill_manual(values = fill_cols, drop = FALSE, name = "Genus") +
    labs(
        title = "CPR - Phytoplankton genus (Top 10 + Other)",
        x = "Longitude",
        y = "Relative Abundance"
    ) +
    theme_minimal() +
...
### Top 10 families (Phytoplankton only)
...
```
::: {.callout-note}
### 💬 Discussion — Dominant phyto Families
